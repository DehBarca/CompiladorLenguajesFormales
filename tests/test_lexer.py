...
class TestLexer:
    ...
    def test_decimal_numbers(self):
        """Verifica tokenización de números decimales."""
        tokens = self.tokenize('{"resultado": 5.9}')
        expected = [
            ('LBRACE', '{'),
            ('STRING', 'resultado'),
            ('COLON', ':'),
    ...
    def test_arrays(self):
        """Verifica tokenización de arrays."""
        tokens = self.tokenize('[1, 2, 3]')
        expected = [
            ('LBRACKET', '['),
            ('NUMBER', 1),
            ('COMMA', ','),
    ...
    def test_complete_blood_analysis_json(self):
        """Verifica tokenización del JSON completo de análisis de sangre."""
        json_text = '''{
            "folio": 15502427,
            "parametros": [
                {"nombre": "Leucocitos", "resultado": 5.9, "limite": "[4.5 - 10.0]"}
            ],
            "firma": {"responsable": "Dr. Test", "cedula": "12345678"}
        }'''
        tokens = self.tokenize(json_text)
        
        # Verificar que contiene todos los tokens necesarios
